...
export default async function mountRight3D(containerId='3d-ampera', modelUrl='/mnt/data/ampera.glb') {
  ...
  catch(e){ console.error('load failed', e); try { const gltf = await loader.loadAsync('/models/ampera.glb'); model = gltf.scene; } catch(err){ console.error('fallback failed', err); } }
