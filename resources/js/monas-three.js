...
export default async function mountRight3D(containerId = "3dmonas") {
  ...
  catch(e){ console.error('load failed', e); try { const gltf = await loader.loadAsync('/models/monas.glb'); model = gltf.scene; } catch(err){ console.error('fallback failed', err); } }
