...
export default async function mountRight3D(containerId='3d-borobudur', modelUrl='/mnt/data/borobudur.glb') {
  ...
  catch(e){ console.error('load failed', e); try { const gltf = await loader.loadAsync('/models/borobudur.glb'); model = gltf.scene; } catch(err){ console.error('fallback failed', err); } }
