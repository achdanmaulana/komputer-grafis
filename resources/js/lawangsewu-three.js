...
export default async function mountRight3D(containerId='3d-lawangsewu', modelUrl='/mnt/data/lawangsewu.glb') {
  ...
  catch(e){ console.error('load failed', e); try { const gltf = await loader.loadAsync('/models/lawangsewu.glb'); model = gltf.scene; } catch(err){ console.error('fallback failed', err); } }
