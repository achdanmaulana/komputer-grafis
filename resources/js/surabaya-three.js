...
export default async function mountRight3D(containerId='3d-surabaya', modelUrl='/models/patung_sura_dan_buaya.glb') {
  ...
  try {
    ...
    try { model = await new Promise((res, rej)=> loader.load('/model/patung_sura_dan_buaya.glb', g=> res(g.scene), undefined, err=> rej(err))); } catch(ee) { model=null; }
  }
